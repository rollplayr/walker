...
CREATE FUNCTION user_bib_activity(
    username_filter TEXT DEFAULT NULL,  -- filter by username instead of UUID
    start_date DATE DEFAULT NULL,       -- filter by record_created_date/updated_date >=
    end_date DATE DEFAULT NULL          -- filter by record_created_date/updated_date <=
)
RETURNS TABLE(
    user_id UUID,
    username TEXT,
...
SELECT
    ...
    COUNT(DISTINCT CASE
        WHEN (i.created_by_user_id = u.id
              AND (start_date IS NULL OR i.record_created_date::date >= start_date)
              AND (end_date IS NULL OR i.record_created_date::date <= end_date))
        THEN i.id END) AS instances_created,

    COUNT(DISTINCT CASE
        WHEN (i.updated_by_user_id = u.id
              AND (start_date IS NULL OR i.updated_date::date >= start_date)
              AND (end_date IS NULL OR i.updated_date::date <= end_date))
        THEN i.id END) AS instances_modified,

    -- holdings
    COUNT(DISTINCT CASE
        WHEN (h.created_by_user_id = u.id
              AND (start_date IS NULL OR h.record_created_date::date >= start_date)
              AND (end_date IS NULL OR h.record_created_date::date <= end_date))
        THEN h.id END) AS holdings_created,

    COUNT(DISTINCT CASE
        WHEN (h.updated_by_user_id = u.id
              AND (start_date IS NULL OR h.updated_date::date >= start_date)
              AND (end_date IS NULL OR h.updated_date::date <= end_date))
        THEN h.id END) AS holdings_modified,

    -- items
    COUNT(DISTINCT CASE
        WHEN (it.created_by_user_id = u.id
              AND (start_date IS NULL OR it.record_created_date::date >= start_date)
              AND (end_date IS NULL OR it.record_created_date::date <= end_date))
        THEN it.id END) AS items_created,

    COUNT(DISTINCT CASE
        WHEN (it.updated_by_user_id = u.id
              AND (start_date IS NULL OR it.updated_date::date >= start_date)
              AND (end_date IS NULL OR it.updated_date::date <= end_date))
